...
/**
* A factory which creates a practiceSession model.
*
* @class Topic
*/
angular.module('tandemWebApp')
  .factory('PracticeSession', [
    ...
    'Recording',
    'session',
    function ($http, $q, Restangular, config, Question, Recording, session) {
      ...
      PracticeSession.create = function (practiceSession) {
        ...
        var request = {
          method: 'POST',
          url: config.API.full + '/practice_sessions',
          headers: {
        ...
        .then(function (response) {
          ...
          var request = {
            method: 'GET',
            url: config.API.full + '/practice_sessions/'+resourceId + '?embedded=' + JSON.stringify(embed),
            headers: {
              'Content-Type': 'application/json'
            }
          };
